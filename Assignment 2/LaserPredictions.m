...
standardlasertrain = (lasertrain - mean(lasertrain))/std(lasertrain);


%Generates a list of lags and neurons with increments of delta given by
%(b-a)/delta + 1 = #of increments
laglist = 75:80;%linspace(10,90,(90-10)/10 + 1);%[8,16,24]%linspace(8,80,(80-8)/8+1); %8-80 ~ten periods of 8, the periodicity of the laser peaks
neuronlist = 49:51;%linspace(10,90,(90-10)/10+1); %10-90 with delta = 10.  testing the amount of effective overparametrization
...
for it = 1:iterations;
    ...
    for neuron = neuronlist;
        lagindex = 1;
        for lag = laglist;
            fprintf("Iteration: %d|Neurons: %d|Lag: %d\n",it,neuron,lag);
            trainingset = lasertrain(1:900);
            valiset = lasertrain(901:end);
            ...
            Error(neuronindex,lagindex) = Error(neuronindex,lagindex) + mse(predictpart,valiset);
...
rng('shuffle')
%Comparing a NN with tunedNeurons (Use 10 neurons and lag 16) and tunedLag with a NN with 10 neurons
%and a lag of 8(one period).
tunedLag = 1;
laglist = [77];
neuronlist = [51];
tunedNeurons = 1;
lag = laglist(tunedLag);
...
net1 = feedforwardnet([neuronlist(tunedNeurons),3],'trainlm');
...
nn2 = feedforwardnet([10,4],'trainlm');
